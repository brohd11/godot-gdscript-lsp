...
Dictionary GDScriptLSPDocument::parse_script(const String &p_script_path) {
    Dictionary out;
    if (!_tree) return out;
    if (_full_revision == get_revision() && _full_path == p_script_path) return _full_cache.duplicate(true);
    Dictionary inherited_constants;
    Dictionary inherited_inner_classes;
    collect_script(ts_tree_root_node(_tree), TSNode{}, _snapshot->source().data(), _src_len,
                   String(), p_script_path, inherited_constants, inherited_inner_classes, out);
    _full_cache = out;
    _full_revision = get_revision();
    _full_path = p_script_path;
    return out.duplicate(true);
}

Dictionary GDScriptLSPDocument::sparse_parse() {
    Dictionary out;
    if (!_tree) return out;
    if (_sparse_revision == get_revision()) return _sparse_cache;
    Dictionary members_out, lines_out;
    collect_symbols(ts_tree_root_node(_tree), TSNode{}, _snapshot->source().data(), _src_len,
                    String(), members_out, lines_out);
    out[Keys::get().members] = members_out;
    out[Keys::get().lines]   = lines_out;
    _sparse_cache = out;
    _sparse_revision = get_revision();
    return out;
}
